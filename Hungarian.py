...
def percolation_finder(m, max_num_percolation=6):
    ...
    def scout(m, breadcrumbs):
        ...
        if crumbs_number < n_cols:
            col_index_arranged = list(
                set(range(n_cols)) - set(breadcrumbs)) + list(set(breadcrumbs))
...
def covering_segments_searcher(m, min_redundancy_percolation):
    ...
    # (B)
    occurrence_vector = [
        min_redundancy_percolation.count(i) for i in range(n_rows)]
    for pos in range(n_rows):
        if occurrence_vector[pos] > 1:
            duplicates_pos = [k for k in range(
                n_rows) if min_redundancy_percolation[k] == pos][1:]
...
def shaker(m, filtered_walks):
    ...
    # (1)
    [cov_row, cov_col] = covering_segments_searcher(
        m, min_redundancy_percolation)
    # (2)
    zero_pos_in_cov_row = [i for i in range(n_rows) if cov_row[i] == 0]
    zero_pos_in_cov_col = [j for j in range(n_cols) if cov_col[j] == 0]
    uncovered_elements = [m[i, j]
                          for i in zero_pos_in_cov_row for j in zero_pos_in_cov_col]
...
def hungarian(m, max_num_percolation=10):
    ...
    while not flag and cont < max_loop:
        ...
        walks = []
        [s, walks] = percolation_finder(
            s, max_num_percolation=max_num_percolation)
        [s, filtered_walks, flag] = resolvability_query(s, walks)
        cont += 1
    walks = []
